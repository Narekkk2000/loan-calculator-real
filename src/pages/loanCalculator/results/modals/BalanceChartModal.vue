<template>
  ...
  >
    ...
    <template #header="{ close }">
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <!-- Legend — mirrors the inline header -->
          <BaseFlex gap="1.5">
            <div class="w-3 h-0.5 rounded-full bg-app-accent" />
            <BaseText tag="span" variant="footnote" color="DIM">Balance over time</BaseText>
          </BaseFlex>
        </div>
    ...
    <!-- ── Body: larger chart ─────────────────────────────────────────────── -->
    <div class="p-5" style="height: 360px">
      <Line
...
import { buildBalanceChartData, buildBalanceChartOptions } from '@/utils/chartConfig'
...
const chartData = computed(() => buildBalanceChartData(props.sampled as any))
const chartOptions = computed(() => buildBalanceChartOptions(props.currency))
</script>
